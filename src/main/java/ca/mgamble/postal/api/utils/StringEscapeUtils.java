...
public class StringEscapeUtils {
    ...
    public static String encodeHtmlExceptAngleBrackets(String source) {
        return encodeHtmlExcept(source, Arrays.asList("&lt;", "&gt;"));
    }

    public static String encodeHtmlAccentedCharacters(String source) {
        Map<Character, String> filteredEncodingTable = htmlEncodeChars
                .entrySet()
                .stream()
                .filter(s -> Arrays.asList("&egrave;").contains(s.getValue()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        return encode(source, filteredEncodingTable);
    }

    public static String encodeHtmlExcept(String source, List<String> exceptions) {
        Map<Character, String> filteredEncodingTable = htmlEncodeChars
                .entrySet()
                .stream()
                .filter(s -> !exceptions.contains(s.getValue()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        return encode(source, filteredEncodingTable);
    }
